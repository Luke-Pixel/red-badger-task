...
} from '../../src/service/parsers.js';
import { Orientation, RobotMovement } from '../../src/types/types.js';
...
describe('parseRobotPosition', () => {
  ...
  it('should throw for invalid format', () => {
    expect(() => parseRobotPosition('invalid', 2)).toThrow(
      'Line: 2, Invalid initial robot position, expected (X Y Orientation(N,E,S,W))'
    );
  });

  it('should throw for negative coordinates', () => {
    expect(() => parseRobotPosition('-1 0 E', 2)).toThrow(
      'Line: 2, Invalid initial robot position, expected (X Y Orientation(N,E,S,W))'
    );
  });
});
